...
//  --lamp[0] is a point-light source at location (5,5,5).
  ...
  this.lightRay = new CRay();	        // the ray from the camera for each pixel
  this.lightRayTwo = new CRay();
...
//      console.log('iNow should be 1; it is:', iNow);
      ...
      //this.item[iNow].rayTranslate(7,1,1.3);        // move drawing axes 
      this.item[iNow].rayTranslate(0,5,1.3);
                                                    // RIGHT, BACK, & UP.
      this.item[iNow].rayRotate(0.40*Math.PI, 1,0,0); // rot 45deg on x axis to face us
      ...
      vec4.set(this.item[iNow].gapColor,  0.0,0.0,1.0,1.0); // RGBA(A==opacity) blue
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
  	  this.item[iNow].rayTranslate(5,1.0,1.3);         // move drawing axes 
  	                                                  // LEFT, BACK, & UP.
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0,-4,1);  // move rightwards (+x),
      ...
      //
            //-----Sphere 2-----
            this.item.push(new CGeom(RT_SPHERE_TWO));       // Append sphere to item[] &
            iNow = this.item.length -1;                 // get its array index.
            // Initially leave sphere at the origin. Once you see it, then
            // move it to a more-sensible location:
            this.item[iNow].setIdent();                   // start in world coord axes
            this.item[iNow].rayTranslate(1,-2,1);  // move rightwards (+x),
            ...
            //
                  //-----Sphere 3-----
      this.item.push(new CGeom(RT_SPHERE_THREE));       // Append sphere to item[] &
      iNow = this.item.length -1;                 // get its array index.
      // Initially leave sphere at the origin. Once you see it, then
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
    ...
    case 1:
      ...
      // move it to a more-sensible location:
      this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-1,0,1);  // move rightwards (+x),
      ...
      //
            ...
            // move it to a more-sensible location:
            this.item[iNow].setIdent();                   // start in world coord axes
            this.item[iNow].rayTranslate(0,0,1);  // move rightwards (+x),
            this.item[iNow].rayScale(0.67,0.67,0.67);
            // and toward camera (-y) enough to stay clear of disks, and up by 1 to
            // make this radius==1 sphere rest on gnd-plane.
            //
            //
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(1,0,1); 
       // move rightwards (+x),
      this.item[iNow].rayScale(0.33,0.33,0.33);
      // and toward camera (-y) enough to stay clear of disks, and up by 1 to
      // make this radius==1 sphere rest on gnd-plane.
      break;
    case 2:
      ...
      //
            ...
            // move it to a more-sensible location:
            this.item[iNow].setIdent();                   // start in world coord axes
            this.item[iNow].rayTranslate(1,-2,4);  // move rightwards (+x),
            this.item[iNow].rayScale(0.75,0.75,0.75);
            // and toward camera (-y) enough to stay clear of disks, and up by 1 to
            // make this radius==1 sphere rest on gnd-plane.
            //
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0,0,2);  // move rightwards (+x),
      this.item[iNow].rayScale(0.25,0.25,0.25);
      // and toward camera (-y) enough to stay clear of disks, and up by 1 to
      // make this radius==1 sphere rest on gnd-plane.
      //
      ...
      // move it to a more-sensible location:
      this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0,0,4);  // move rightwards (+x),
      // and toward camera (-y) enough to stay clear of disks, and up by 1 to
      // make this radius==1 sphere rest on gnd-plane.
      //
      //
      ...
      //
            ...
            // move it to a more-sensible location:
            this.item[iNow].setIdent();                   // start in world coord axes
            this.item[iNow].rayTranslate(0,2,2);  // move rightwards (+x),
            this.item[iNow].rayScale(0.67,0.67,0.67);
            // and toward camera (-y) enough to stay clear of disks, and up by 1 to
            // make this radius==1 sphere rest on gnd-plane.
            //
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0,0,0.5); 
       // move rightwards (+x),
      this.item[iNow].rayScale(0.33,0.33,0.33);
      // and toward camera (-y) enough to stay clear of disks, and up by 1 to
      // make this radius==1 sphere rest on gnd-plane.
...
//	console.log("You called CScene.makeRayTracedImage!")
  ...
  for(j=0; j< this.imgBuf.ySiz; j++) {        // for the j-th row of pixels.
      ...
      // DIAGNOSTIC:------------------------------------
      if(i==this.imgBuf.xSiz/2 && j==this.imgBuf.ySiz/2) { 
      ...
      colr = this.traceRay(this.eyeRay);
/*
      ...
      if(((j+1) % 4 == 0) && ((i+1) % 4 == 0)){
        // Set pixel color in our image buffer------------------------------------
        idx = (j*this.imgBuf.xSiz + i)*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR += this.imgBuf.fBuf[idx   ];
        avgG += this.imgBuf.fBuf[idx +1];
        avgB += this.imgBuf.fBuf[idx +2];
  
        idx = ((j-1)*this.imgBuf.xSiz + i)*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-2)*this.imgBuf.xSiz + i)*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-3)*this.imgBuf.xSiz + i)*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j)*this.imgBuf.xSiz + (i-1))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-1)*this.imgBuf.xSiz + (i-1))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-2)*this.imgBuf.xSiz + (i-1))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-3)*this.imgBuf.xSiz + (i-1))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j)*this.imgBuf.xSiz + (i-2))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-1)*this.imgBuf.xSiz + (i-2))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-2)*this.imgBuf.xSiz + (i-2))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-3)*this.imgBuf.xSiz + (i-2))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j)*this.imgBuf.xSiz + (i-3))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-1)*this.imgBuf.xSiz + (i-3))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-2)*this.imgBuf.xSiz + (i-3))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
  
        idx = ((j-3)*this.imgBuf.xSiz + (i-3))*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        avgR +=this.imgBuf.fBuf[idx   ];	
        avgG +=this.imgBuf.fBuf[idx +1];
        avgB +=this.imgBuf.fBuf[idx +2];
        ...
        idx = (j*this.imgBuf.xSiz + i)*this.imgBuf.pixSiz;	// Array index at pixel (i,j) 
        this.imgBuf.fBuf[idx   ] = avgR;	
        this.imgBuf.fBuf[idx +1] = avgG;
        this.imgBuf.fBuf[idx +2] = avgB;
...
CScene.prototype.traceRay = function(ray) {
  ...
  for(var k=0; k<this.item.length;k++) {
    this.item [k].traceMe(ray, myHit);
  }
    //console.log("peep the recurse" + recurseLevel);
    return this.findShade(myHit, ray);
  

  }
  
  CScene.prototype.findShade = function(myHit, ray) {
    /* 
    A lot of credit for my shadow and phong lighting code ought to go to Ikenna, firstly, but also 
    to Jipeng and Prof. Tumblin who made a tutorial video of sorts out of an
    office hours session with him that was instrumental for me. 
    */
    var colr = vec4.fromValues(0, 0, 0, 1);
    var N = myHit.surfNorm;
    N[3] = 0;
    vec4.normalize(N,N);
    var V = myHit.viewN;
    V[3] = 0;
    ...
    var shadow = false;
    var newHit = new CHit();
    this.lightRay.orig = vec4.fromValues(lightOneX, lightOneY, lightOneZ, 1);
    vec4.subtract(this.lightRay.dir, this.lightRay.orig, myHit.hitPt);      // directed to light source
    this.lightRayTwo.orig = vec4.fromValues(lightTwoX, lightTwoY, lightTwoZ, 1);
    vec4.subtract(this.lightRayTwo.dir, this.lightRayTwo.orig, myHit.hitPt);      // directed to light source

    if (myHit.hitNum == 2){
    var ka = vec3.fromValues(0.19125,  0.0735,   0.0225);
    var kd = vec3.fromValues(0.7038,   0.27048,  0.0828);
    var ks = vec3.fromValues(0.256777, 0.137622, 0.086014);
    }
    else if(myHit.hitNum == 7){
      var ka = vec3.fromValues(0.25,     0.20725,  0.20725);
      var kd = vec3.fromValues(1.0,      0.829,    0.829);
      var ks = vec3.fromValues(0.296648, 0.296648, 0.296648);
    }
    else if(myHit.hitNum == 8){
      var ka = vec3.fromValues(0.02,    0.02,   0.02);
      var kd = vec3.fromValues(0.01,    0.01,   0.01);
      var ks = vec3.fromValues(0.4,     0.4,    0.4);
    }
    else if(myHit.hitNum==1 || myHit.hitNum==0){
      var ka = vec3.fromValues(0.2, 0.2, 0.2);
      var kd = vec3.fromValues(1, 1, 1);
      var ks = vec3.fromValues(1, 1, 1);
    } 
    else if(myHit.hitNum== -1){
      var ka = this.skyColor;
      var kd = this.skyColor;
      var ks = this.skyColor;
    } 

    //var lightList = [this.lightRay, this.lightRayTwo];
    //for(var i = 0; i<2; i++){
      if(lightOneOn){
        var la = vec3.fromValues(0.2, 0.2, 0.2);
        var ld = vec3.fromValues(1.0, 1.0, 1.0);
        var ls = vec3.fromValues(1.0, 1.0, 1.0);
        var feelerRay = new CRay(); 
        feelerRay.orig[0] = this.RAY_EPSILON * this.lightRay.dir[0]*10000;
        feelerRay.orig[1] = this.RAY_EPSILON * this.lightRay.dir[1]*10000;
        feelerRay.orig[2] = this.RAY_EPSILON * this.lightRay.dir[2]*10000;
        feelerRay.orig[3] = 0;
        vec4.subtract(feelerRay.orig, myHit.hitPt, feelerRay.orig);
        vec4.subtract(feelerRay.dir, this.lightRay.orig, myHit.hitPt);      // directed to light source
        
        newHit.init();
    
    
        for(var k = 0; k < this.item.length; k++) {
            if (this.item[k] == myHit.hitGeom) continue;
                this.item[k].traceMe(feelerRay, newHit);
                if(newHit.hitGeom != -1){
                  shadow = true;
                }
          ...
          var la = vec3.fromValues(0,0,0);
          var ld = vec3.fromValues(0,0,0);
          var ls = vec3.fromValues(0,0,0);  
        }
        if(lightTwoOn){
          var la2 = vec3.fromValues(0.4, 0.1, 0.4);
          var ld2 = vec3.fromValues(0.3, 0.7, 0.1);
          var ls2 = vec3.fromValues(0.8, 1.0, 0.1); 
          var feelerRay = new CRay(); 
          feelerRay.orig[0] = this.RAY_EPSILON * this.lightRayTwo.dir[0]*10000;
          feelerRay.orig[1] = this.RAY_EPSILON * this.lightRayTwo.dir[1]*10000;
          feelerRay.orig[2] = this.RAY_EPSILON * this.lightRayTwo.dir[2]*10000;
          feelerRay.orig[3] = 0;
          vec4.subtract(feelerRay.orig, myHit.hitPt, feelerRay.orig);
          vec4.subtract(feelerRay.dir, this.lightRayTwo.orig, myHit.hitPt);      // directed to light source
          ...
          for(var k = 0; k < this.item.length; k++) {
            ...
            var la2 = vec3.fromValues(0,0,0);
            var ld2 = vec3.fromValues(0,0,0);
            var ls2 = vec3.fromValues(0,0,0);  
            
          }
              colr[0] = ka[0] * la[0] + ka[0] * la2[0];
    colr[1] = ka[1] * la[1] + ka[1] * la2[1];
    colr[2] = ka[2] * la[2] + ka[2] * la2[2];


    if(!shadow){
      var L = this.lightRay.dir;
      L[3] = 0;
      ...
      var lambertian = Math.max(NL, 0.0);
   //   if(lambertian > 0.0){
      ...
      vec4.normalize(R,R);
      var RV = vec4.dot(R, V);
      var shininessVal = 15.0;
      var specAngle = Math.max(RV, 0.0);
      var specular = Math.pow(specAngle, shininessVal);
      if(this.pixFlag == 1){
      console.log(i);
    }
      colr[0] += // ka[0] * la[0] + 
      ...
      * specular * ls[0];
      ...
      /*
       ...
       if(myHit.hitNum==0) {  // use myGrid tracing to determine color
        ...
        vec4.multiply(colr, colr, myHit.hitGeom.gapColor);
      }
      else if (myHit.hitNum==1) {
        vec4.multiply(colr, colr, myHit.hitGeom.lineColor);
      }
      else if (myHit.hitNum == -1){
        vec4.multiply(colr, colr, this.skyColor);
      }
    }
  ...
  //  }
    ...
    //}
    if(recurseLevel >= recurseDepth){
      recurseLevel = 0;
        return colr;
      }
    var reflectRay = new CRay();
      
    reflectRay.orig[0] = this.RAY_EPSILON * N[0]*10000;
    reflectRay.orig[1] = this.RAY_EPSILON * N[1]*10000;
    reflectRay.orig[2] = this.RAY_EPSILON * N[2]*10000;
    reflectRay.orig[3] = 0;
    
    vec4.subtract(reflectRay.orig, myHit.hitPt, reflectRay.orig);
    
    // reflectRay.orig = myHit.hitPt - this.RAY_EPSILON;
    //reflectRay.dir = vec4.fromValues(0, 1, 0, 0);
    var VN = vec4.dot(V, N);
    reflectRay.dir = N;
    reflectRay.dir[0] *= -2*VN;
    reflectRay.dir[1] *= -2*VN;
    reflectRay.dir[2] *= -2*VN;
    
    vec4.subtract(reflectRay.dir, V, reflectRay.dir);
    reflectRay.dir[3] = 0;
    
    //  console.log("peep the dir " + reflectRay.dir);
    
    recurseLevel +=1;
    if(this.pixFlag == 1){
      
      console.log("rray orig " + reflectRay.orig);
      console.log("og hitpoint " + myHit.hitPt);
      console.log(" shapetype " + CGeom.shapeType);
    }
 // if(recurseLevel <= 1){
 vec4.add(colr, colr, this.traceRay(vec4.normalize(reflectRay,reflectRay)));
  colr[0] = Math.min(colr[0], 1.0);
  colr[0] *= 0.7;
  colr[1] =  Math.min(colr[1], 1.0);
  colr[1] *= 0.7;
  colr[2] =  Math.min(colr[2], 1.0);
  colr[2] *= 0.7;
  colr[3] = 1;
