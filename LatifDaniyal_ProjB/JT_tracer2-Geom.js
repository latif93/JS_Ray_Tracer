...
const RT_SPHERE_TWO   = 7;
const RT_SPHERE_THREE   = 8;
...
// ellipsoids for the unit sphere and rectangles (or prisms) from the unit box.
	...
	switch(this.shapeType) {
	  ...
	  case RT_SPHERE: //----------------------------------------------------------
    	...
    	this.sphereColor = vec4.fromValues(1.0,1.0,1.0,1.0);  // RGBA blue(A==opacity)
	    break;
    case RT_SPHERE_TWO: //----------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceSphere2(inR,hit); }; 
    	this.sphereColor = vec4.fromValues(1.0,1.0,1.0,1.0);  // RGBA blue(A==opacity)
	    break;
      case RT_SPHERE_THREE: //----------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceSphere3(inR,hit); }; 
    	this.sphereColor = vec4.fromValues(1.0,1.0,1.0,1.0);  // RGBA blue(A==opacity)
	    break;
	  case RT_BOX:    //----------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
...
//------------------ Step 2: Test 1st triangle. Did ray MISS sphere entirely?
  ...
  // in model space we know it's always +z,
  // but we need to TRANSFORM the normal to world-space, & re-normalize it.
  vec4.transformMat4(myHit.surfNorm, vec4.fromValues(myHit.modelHitPt[0],myHit.modelHitPt[1],myHit.modelHitPt[2],0), this.normal2world);
  vec4.normalize(myHit.surfNorm, myHit.surfNorm);
  // TEMPORARY: sphere color-setting
  myHit.hitNum = 2;   // in CScene.makeRayTracedImage, use 'this.sphereColor'
   // DIAGNOSTIC:---------------------------------------------------------------
  if(g_myScene.pixFlag ==1) {   // did we reach the one 'flagged' pixel
                                // chosen in CScene.makeRayTracedImage()?
...
CGeom.prototype.traceSphere2 = function(inRay, myHit) { 
  //==============================================================================
  // Find intersection of CRay object 'inRay' with sphere of radius 1 centered at
  // the origin in the 'model' coordinate system. 
  //
  ...
  */
  ...
  //------------------ Step 2: Test 1st triangle. Did ray MISS sphere entirely?
    ...
    vec4.normalize(myHit.surfNorm, myHit.surfNorm);
    // TEMPORARY: sphere color-setting
    myHit.hitNum = 7;   // in CScene.makeRayTracedImage, use 'this.sphereColor'
     // DIAGNOSTIC:---------------------------------------------------------------
    if(g_myScene.pixFlag ==1) {   // did we reach the one 'flagged' pixel
                                  // chosen in CScene.makeRayTracedImage()?
    console.log("r2s:", r2s, "L2", L2, "tcaS", tcaS, "tca2", tca2, 
  ...
  CGeom.prototype.traceSphere3 = function(inRay, myHit) { 
    //==============================================================================
    // Find intersection of CRay object 'inRay' with sphere of radius 1 centered at
    // the origin in the 'model' coordinate system. 
    //
    ...
    */
    ...
    //------------------ Step 2: Test 1st triangle. Did ray MISS sphere entirely?
      ...
      vec4.normalize(myHit.surfNorm, myHit.surfNorm);
      // TEMPORARY: sphere color-setting
      myHit.hitNum = 8;   // in CScene.makeRayTracedImage, use 'this.sphereColor'
       // DIAGNOSTIC:---------------------------------------------------------------
      if(g_myScene.pixFlag ==1) {   // did we reach the one 'flagged' pixel
                                    // chosen in CScene.makeRayTracedImage()?
      console.log("r2s:", r2s, "L2", L2, "tcaS", tcaS, "tca2", tca2, 
